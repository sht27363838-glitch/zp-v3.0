...
export default function ReportPage(){
  ...
  // 원본 → 최근 500 → 필터
  const baseRows = useMemo(()=> (data.rows as CsvRow[]).slice(-500).reverse(), [data.rows])
  ...
  // 합계행(필터 반영)
  const sum = (key: keyof CsvRow)=> filtered.reduce((s,r)=> s + num(r[key]), 0)
  const footer = (
    <div className="row" style={{justifyContent:'flex-end', gap:16, fontWeight:600}}>
      <span className="muted">합계</span>
      <span>방문 {fmt(sum('visits'))}</span>
      <span>클릭 {fmt(sum('clicks'))}</span>
      <span>주문 {fmt(sum('orders'))}</span>
      <span>매출 {fmt(sum('revenue'))}</span>
      <span>광고비 {fmt(sum('ad_cost'))}</span>
      <span>반품 {fmt(sum('returns'))}</span>
    </div>
  )
  ...
  const clearFilters = ()=>{ setQuery(''); setFrom(''); setTo(''); setSel(new Set()) }
    ...
    <div className="page">
      ...
      <div className="card" style={{display:'flex', flexWrap:'wrap', gap:8, alignItems:'center', marginBottom:10}}>
        <div className="row" style={{gap:6, flexWrap:'wrap', alignItems:'center'}}>
          <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="채널 검색"
        ...
        <div style={{display:'flex', gap:6, flexWrap:'wrap', alignItems:'center'}}>
          {channels.map(ch=>{
            const on = sel.has(ch)
            return (
              <button key={ch} className="badge" onClick={()=>toggleChannel(ch)}
                style={{
                  cursor:'pointer',
                  borderColor: on? 'var(--primary)' : 'rgba(0,0,0,.12)',
