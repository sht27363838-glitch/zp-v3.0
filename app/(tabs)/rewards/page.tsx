...
export default function RewardsPage() {
  ...
  const guards = evalGuards({ revenue, ad_cost: adCost, orders, visits, returns, ctr: CTR, freq: undefined }, rules)
      ...
      <div className="mt-6">
        ...
        <div style={{ maxHeight: 420, overflow: 'auto' }}>
          <table className="table">
            ...
            <tbody>
              {data.rows.slice(-50).reverse().map((r:any, i:number)=>(
                <tr key={i}>
                  <td>{r.date}</td>
                  <td>{r.mission}</td>
                  <td>{r.type}</td>
